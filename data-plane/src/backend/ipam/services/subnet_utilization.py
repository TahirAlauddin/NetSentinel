        ...
        projected_used.append(
            {
                ...
                "projected_utilization": (
                    round((projected / usable_hosts) * 100, 2) if usable_hosts > 0 else 0
                ),
            }
        )
...
def get_subnet_utilization_trends(
    subnet: Subnet, days: int = 30, interval: str = "day"
) -> List[Dict]:
    """
...
def get_all_subnets_utilization(
    filters: Optional[Dict] = None, threshold: Optional[float] = None
) -> List[Dict]:
    """
...
def get_utilization_summary() -> Dict:
    ...
    overall_utilization = (total_used / total_usable * 100) if total_usable > 0 else 0
